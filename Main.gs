...
function verificarSemana() {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    ss.toast("Iniciando verificación semanal... (puede tomar 2-3 minutos)", "🔍", 180);

    // 1: Leer snapshot base del Top Candidatos
    ...
    for (var ti = 0; ti < topData.length; ti++) {
        ...
        if (!tk) continue;
        var scoreBase = parseFloat(topData[ti][COL_SCORE_TOP - 1]) || 0;
        var fechaBase = topData[ti][COL_SNAPDATE - 1] || "";
        var filtrosBase = [];
        for (var fi = 0; fi < FILTROS.length; fi++) {
            if (String(topData[ti][8 + fi]).trim() === "✓") filtrosBase.push(fi);
        }
        snapshotBase[tk] = { scoreBase: scoreBase, filtrosBase: filtrosBase, fechaBase: fechaBase };
    }
    ...
    var nuevas = [], mejoraron = [], estables = [], deterioraron = [], desaparecieron = [];
    ...
    for (var bi = 0; bi < tickersBase.length; bi++) {
        ...
        if (mapaActual[tkB]) {
            ...
            var obj = {
                ...
                filtrosActual: mapaActual[tkB].filtros
            };
            if (delta > 0.5) mejoraron.push(obj);
            else if (delta < -0.5) deterioraron.push(obj);
            else estables.push(obj);
        } else {
            desaparecieron.push({ ticker: tkB, scoreBase: snapshotBase[tkB].scoreBase, delta: -snapshotBase[tkB].scoreBase });
        }
    }

    for (var ni = 0; ni < tickersActuales.length; ni++) {
        var tkN = tickersActuales[ni];
        if (!snapshotBase[tkN] && mapaActual[tkN].scoreActual >= 3) {
            nuevas.push({
                ...
                price: mapaActual[tkN].price, filtrosActual: mapaActual[tkN].filtros
            });
        }
    }

    function byScore(a, b) { return b.scoreActual - a.scoreActual; }
    nuevas.sort(byScore); mejoraron.sort(function(a,b){return b.delta-a.delta;});
    ...
    var row = 5;
    row = escribirBloqueVerif(wsVer, "⭐ NUEVAS ENTRADAS", "#1B5E20", "#FFFFFF", nuevas, row);
    ...
    row = escribirBloqueVerif(wsVer, "🔴 DESAPARECIERON", "#B71C1C", "#FFFFFF", desaparecieron, row);

    if (errores.length > 0) {
        wsVer.getRange(row, 1, 1, 10).merge().setValue("⚠️ Filtros con error: " + errores.join(" | ")).setBackground("#FFF9C4");
    }
...
function escribirBloqueVerif(ws, titulo, bgTitulo, fgTitulo, items, row) {
    ...
    ws.getRange(row, 1, 1, 10).merge().setValue(titulo + " (" + items.length + ")").setBackground(bgTitulo).setFontColor(fgTitulo).setFontWeight("bold");
    row++;

    if (items.length === 0) {
        ws.getRange(row, 1, 1, 10).merge().setValue("— ninguno —").setBackground("#F5F5F5").setHorizontalAlignment("center");
        return row + 2;
    }

    var hdrs = ["Ticker", "Empresa", "Sector", "Score Base", "Score Ahora", "Δ Score", "Filtros", "Perf Sem%", "Perf Mes%", "Precio"];
    ...
    for (var ii = 0; ii < items.length; ii++) {
        ...
        ws.getRange(row, 1, 1, 10).setBackground(bg).setFontSize(9).setHorizontalAlignment("center");
        ...
        var fNames = it.filtrosActual ? it.filtrosActual.map(function(f){ return FILTROS[f].key.substring(0,6); }).join(", ") : "";
