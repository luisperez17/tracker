...
function guardarScoreLogV4(ss, datos) {
    ...
    if (filas.length > 0) {
        ...
        log.getRange(startRow, 1, filas.length, 8).setValues(filas);
        // Formatear fechas
...
function onEditTrackerV4(e) {
    ...
    try {
        ...
        // Detectar si pegó un rango o escribió una celda
        var valores;
        if (e.value && typeof e.value === 'object' && Array.isArray(e.value)) {
            ...
            // Fallback: leer directamente del rango (funciona siempre, incluso en paste)
