...
function formatearHojaSemana(ws) {
    ...
    for (var r = 0; r < MAX_CAND; r++) {
        ...
        ws.getRange(rowNum, 10).setFormula('=IF(OR(' + E + '="";' + S_ + '="");"";"1:2.00")').setHorizontalAlignment("center");
    }
    ws.setFrozenRows(5);
}
...
/**
 * Formatea la hoja "Score Log".
 */
function formatearScoreLog(ws) {
    ws.getRange(1, 1, 1, 10).merge().setValue("📊  SCORE LOG HISTÓRICO").setBackground(C.DARK).setFontColor(C.GREEN).setFontWeight("bold").setHorizontalAlignment("center");
    var hdrs = ["FECHA", "TICKER", "P. 10AM", "SCORE", "FILTROS", "P. 11AM", "P. 1PM", "P. 2:30PM", "P. 3:45PM", "TRACKER?"];
    for (var i = 0; i < hdrs.length; i++) {
...
function formatearHojaReporte(ws) {
    ...
    ws.setColumnWidth(7, 80);  // P. Máximo
    ws.setColumnWidth(8, 80);  // PnL %
    ws.setColumnWidth(9, 120); // Estatus

    // --- ENCABEZADO ---
    ws.getRange(1, 1, 1, 9).merge().setValue("📊 REPORTE DE RENDIMIENTO SEMANAL (SCORE LOG)")
    ...
    ws.getRange(5, 2, 9, 1).setHorizontalAlignment("center").setFontWeight("bold");

    // --- ENCABEZADOS DE TABLA DETALLADA ---
    var headers = ["FECHA", "TICKER", "SCORE PROM", "FILTROS ACTIVOS", "P. INICIO", "P. FIN", "P. MÁXIMO", "PnL %", "ESTATUS"];
